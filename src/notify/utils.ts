import { setPageHidden, PageHidden, queueRenderAll, Store } from "./api";

export const animationEndEvents = "webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend";

export function inArray(needle: any, haystack: any, argStrict?: any) {
  let key;
...
export const deepExtend = function (out, ...args) {
  out = out || {};
...
export function addListener(el, events, cb?: Function, useCapture = false) {
  events = events.split(" ");
    ...
    setTimeout(function () {
      Object.keys(Store).forEach((id) => {
        if (Store.hasOwnProperty(id)) {
          if (Store[id].options.visibilityControl) {
            Store[id].stop();
          }
        }
      });
