...
const defaultConfig: Partial<Omit<Options, "text">> = {
  theme: "metroui",
  timeout: 1000
};

export const notify = (message: string, type?: Type, config: Omit<Options, "text"> = {}) => {
  new Noty({ ...defaultConfig, ...config, ...{ text: message, type } }).show();
};
...
export default class Noty {
  ...
  show() {
    ...
    if (this.options.animation.open === null) {
      this.promises.show = new Promise((resolve: any) => {
        resolve();
      });
  ...
  close() {
    ...
    if (this.options.animation.close === null || this.options.animation.close === false) {
      this.promises.close = new Promise((resolve: any) => {
        resolve();
      });
  ...
  static overrideDefaults(obj) {
    API.setDefault(Utils.deepExtend({}, API.Defaults, obj));
    return this;
  }
